...
\item $S$ is convex balanced such that $\lambda S=S\qquad (\lambda > 0)$
\end{enumerate}
\renewcommand{\labelenumi}{(\alph{enumi})}
%
...
%
\begin{equation}\label{induction nS}
  nS = (n-1)  S + S = S + S = S \qquad (n=1,2,3, \dots)
\end{equation}
%
...
\\
Let $V$ be a collection of vector subspaces of $X$, %
...
\end{equation}
%
(the fact that $I  = I + \{0\} \subset I + I$ was tacitly used). %
%
It now follows from the implication (ii) $\Rightarrow$ (i) of the lemma that %
...
\end{equation}
%
It then follows from the lemma's (ii) implication $\Rightarrow$ (i) that $U$ is a vector subspace of $X$. %
%
...
Finally, we conclude from (ii) that any $A+B$ is a vector subspace of $X$. %
%
So ends the proof. %
\end{enumerate}
\end{proof}
