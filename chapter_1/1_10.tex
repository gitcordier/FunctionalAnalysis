...
%
\begin{proof}%
The case $Y=\singleton{0}$ is trivial. Let $Y$ have dimension $n>0$ and basis $B=\singleton{e, e', \dots}$. Let $W\subset X$ %
be a neighborhood of the origin. Since addition is continuous in $X$, there exists a balanced open $V$ such that
%
  \begin{equation}
  ...
  \end{equation}
%
Note that $\Lambda(V)$ is balanced as well. Moreover, by the surjectivity of $\Lambda$, there exists a vector $x_e\in X$ %
such that $\Lambda x_e = e$. Therefore, given the coordinate representation $\sum_e y_e e$ of $y\in Y$, we have
%
\begin{equation}\label{1_10_sum}
  y = \sum_e y_e e = \sum_e y_e \Lambda x_e. 
\end{equation}
%
Note that the finite set $\set{x_e}{e\in B}$ is bounded. Hence %
%
\begin{equation}
  \set{x_e}{e \in B} \subset sV 
\end{equation}
%
for some $s > 0$. Equation (\ref{1_10_sum}) then yields % 
%
...
%
  ...
  \end{equation}
%
This proves (a) for $Y = \C^n$ equipped with $\norm*[\infty]{\cdot}$ and the standard basis. The general case now follows from [\ref{notations: vector spaces: finite-dimensional vector spaces}]. 

To prove (b), assume that the null space $N = \set{x}{\Lambda x = 0}$ is closed and let $f, \pi$ be as in Exercise 1.9. %
