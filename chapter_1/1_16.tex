...
definition with a well-known result (for instance, see \citebook{2.6}{BigRudin}) 
...
specified in section 1.44, then every compact set $K$ lies in almost all 
%
  $\mathit{Q^{\,\circ}_n}$, \ie, 
%
...
%
  ...
  \end{equation}
%
yields a decreasing sequence of compact\footnote{
  See \citebook{(b) of 2.5}{BigRudin}.
} 
sets $\singleton{C_n}$. Suppose (to reach a contradiction) that 
% 
  no $C_n$ is empty 
% 
and so conclude\footnote{
  ...
  This is a corollary of \citebook{2.6}{BigRudin}.
} 
...
%
  ...
  \end{equation} 
%
We now reiterate the above proof with $K_n$ and $L_n$ in exchanged roles 
