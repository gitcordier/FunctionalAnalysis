...
{\CMUCS Suppose $d_{1}(x,y)=|x-y|, d_{2}(x,y)=|\phi(x)-\phi(y)|$, where $\phi(x)={x}/{(1+\lvert x \rvert )}$. Prove that $d_{1}$ and $d_{2}$ are metrics on $\R$ which induce the same topology, although $d_{1}$ is complete and $d_{2}$ is not.}
