...
\IFF is a shorthand for ``if and only if". Splitting \IFF into \emph{if-then} clauses shows that it is  a rewording of the %
logical equivalence $\iff$. All definitions will use the \emph{if and only if} convention, which is consistent with the fact %
that every definition expresses an identity.
%
\subsection{The assignment operator $\Def$}%
Given $\varit{a}$ and $\varit{b}$, $\triangleq$ is a specialization of $=$. We say that $a\triangleq b$ \IFF $a$ and $b$ are %
\emph{assumed} to be equal. Typically, $a\triangleq b$ means that $a$ is assigned the previously known value $b$ %
(some authors write $a:=b$) but this is not a strict requirement. 
%
\subsection{Iverson brackets}\label{Iverson-notation}
Given a boolean expression $\phi$, $\Iverson{\phi}$ encodes the truth value of $\phi$ as a {\fw bit}:
%
\begin{align} \nonumber
...
\end{align}
%
For example, $\Iverson{1 > 0} = 1$ but $\Iverson{ \sqrt{2} \in \Q} = 0$. For interpretations of Boolean operators in logic, %
see \cite{SpecifyingSystems}.
% 
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% SETS
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
restriction $X \neq Y$ will be explicitly stated when it applies.
%
Given $A$ and $B$, $A\cup B$ is the union of $A$ and $B$. More generally, for any collection $C$ of set(s) $S$, the union $U$ %
of $C$ is expressed as follows:
%
\begin{equation}
U \Def \bm{\bigcup}C \Def \bigcup_{S \in C} S.
\end{equation}
%
...
In a metric space context, $X\equiv Y$ means that $\phi$ is an onto isometry. %
%
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% TVS 
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
case is therefore \emph{the stronger one}. This restriction may be significant in some contexts. However, the %
standard scalar field is $\C$. Considering $\R$ instead of $\C$ makes no difference, unless stated otherwise, as in %
\citeFA{3.1 and Chapter 12}.%
%
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Vector spaces
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
%
\begin{align}
  \set{(z_u)_{u\in B}}{z_u \in \C, \set{u}{z_u \neq 0} \text{ is finite}} & \to X \\\nonumber
  (z_u) & \mapsto \sum_{z_u \neq 0 } z_u u
\end{align}
...
\end{enumerate}
%
We now turn to the finite-dimensional case over the field $\C$. The zero-dimensional case is %
$B=\emptyset$\ie $X=\singleton{0}$. Our first step is to study $\C^n$ ($n>0$) the standard $n$-dimensional vector space.
%
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Finite dimension
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
$\C^n$ has a standard basis $1_{\singleton{0}}, \dots, 1_{\singleton{n-1}}$ so that scalar $z_k$ is the $k$-th component of %
%
\begin{equation}
  (z_1,\dots, z_n) \Def 
  z_1 \cdot (\,\underbrace{1, 0, \dots, 0}_{1_{\singleton{0}}}\,)
  + \cdots
  + z_n  \cdot (\,\underbrace{0, \dots,0,  1}_{1_{\singleton{n-1}}}\,), 
\end{equation}
as $z=(z_1, \dots, z_n)$ ranges over $\C^n$. Moreover, $\C^n$ is endowed with the topology generated by all polydiscs %
%
\begin{equation}
...
see [\ref{Euclidean-norm}]. %
%
To show this, choose an isomorphism $f: \C^n \to Y$. For instance, let $f(e_k)$ be $u_k$, as in \citeFA{1.20}, when %
$\{u_k\}$ is a basis of the $n$-dimensional $Y$, see [\ref{vector-space-basis}]. It follows from \citeFA{1.21} that $f$ is a %
homeomorphism. A striking consequence is that %
%
  \emph{$\set{f(U)}{U \text{ open in }\C^n}$ is the only topological vector space topology for $Y$}. %
%
in other words, there is only one version of $Y$ as a topological vector space. Thus, $Y$ is necessarily locally convex and %
locally bounded \ie normable, see \citeFA{1.39}. Note that $\norm[Y]{y}= \norm[2]{f^{\minus 1}(y)}$ for all $y\in Y$ is an%
example of a norm. Additionally, $Y$ is locally compact, as the closed unit ball of $\C^n$ is compact. %
Choose an $n$-dimensional topological vector space $W$, and repeat the same reasoning, first with $g: \C^n \to W$, next with %
$h = g\circ f^{\,\minus 1}$, playing the role of $f$. This establishes that the homeomorphism $h$ maps $Y$ onto $W$ and that %
$W$ is normable as well. The following assertions are then equivalent in the finite-dimensional context: %
%
\begin{enumerate}
  \item $\dim{W} = \dim{Y},$ 
...
any given norm $\normtriple{W}{\cdot}$ on $W$, there exists a positive constant (termed the \emph{modulus of continuity}) %
$C= C_h$ such that %
%
\begin{align}
...
\end{equation}
%
is easily derived from definitions - see \citeFA{1.19} - with the exception of $C_{2, 1} = \sqrt{n}$. Indeed, %
$\norm[1]{z} \leq \sqrt{n} \norm[2]{z}$ is a Cauchy-Schwarz inequality, see (1) in \citeFA{12.2}. The table below records %
these sharp $C_{i,j}$,
%
...
\begin{enumerate}\setcounter{enumi}{1}
  \item For each compact $K \subset X$ there exists a modulus of continuity $M_K$ such that %
  \begin{equation*}
...
% 
Condition (ii) defines Radon measures in a weaker sense, see \cite{AnalyseIII}. Moreover, $\Lambda$ is bounded on $C_c(X)$ %
with respect to the supremum norm \IFF (ii) is satisfied with all $M_K\leq \norm[\infty]{\Lambda} <\infty$. These bounded %
functionals are then uniformly continuous Radon measures. According to \citebook{6.19}{BigRudin}, each of them is %
isometrically identified with a specific regular Borel measure $\mu$. Conversely, when the ambient space $X$ is compact, %
every Radon measure is $\norm[\infty]{\cdot}$-bounded. For instance, this case is addressed in Exercise 3 of Chapter 2. %
In measure theory context, by \emph{support} of $\mu$, we mean: 
%
\begin{equation}
...
specific positive and regular Borel measure $\beta:X \to \R$. Hence % 
%
\begin{equation}\label{definition-of-sum}
	\int_{X} \phi \diff{\beta} \Def \Lambda \phi \qquad \bigl(\phi \in C_c(X)\bigr),
\end{equation}
%
where the left side is a Lebesgue integral. The regularity property implies that $\beta$ has total mass %
%
\begin{equation}
	\int_{X} 1 \diff{\beta} \Def \sup \set{\Lambda \phi}{\phi \in C_c(X), \norm[\infty]{\phi} \leq 1}.
\end{equation}
%
...
\end{equation}
%
An important issue in calculus is to know whether the modulus integral $\magnitude{f}_1$, namely 
%
\begin{equation}
  \magnitude{f}_1 \Def \int_{X}\magnitude{f}\diff{\beta} \qquad \left(f \text{ $X$-measurable}\right), 
\end{equation}
%
is finite. This justifies the following definitions:%
%
\begin{equation}
	\mathcal{L}^1(X, \beta)\Def \set{f}{\magnitude{f}_1  < \infty} 
  \subset \mathcal{L}^1_{\text{loc}}(X,\beta) 
  \Def \bm{\bigcap} \set[\big]{\mathcal{L}^1(K,\beta)}{K \subset X \text{ is compact.}}
\end{equation}
%
...
%
\begin{align}
  \mathcal{L}^p &\Def \set{f}{\magnitude{f}^p \in \mathcal{L}^1}, \\
  L^p & \Def \mathcal{L}^p / N,  \\
  \norm[p]{f+N} &\Def \sqrt[p]{\magnitude{{\magnitude{f}^p}}}_1 \qquad (f \in \mathcal{L}^p).
\end{align}
%
Similar \emph{quasi-Banach} $L^p$ spaces exist for $0< p < 1$, with the noticeable difference that %
$\sqrt[p]{\magnitude{{\magnitude{f}^p}}}_1$ no longer defines a norm (the triangle inequality is lost). In contrast, %
the counterpart $L^\infty = \set{f+N}{\norm[\infty]{f+N} < \infty}$ is equipped with $\norm[\infty]{\cdot}$, where the %
quotient-norm
%
\begin{equation}
  \norm[\infty]{f+N} \Def \inf\set{M}{M > 0, \magnitude{f} < M \;\,\beta \text{-a.e.}}
\end{equation}
%
over bounded functions $f$ turns it into a Banach space.%
%
% END 
%
%