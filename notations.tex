...
$a\triangleq b$ is inconsistent whether $a\neq b$.
...
Provided a pair $(A, X)$, $\subset$ and $\supset$ are the regular symbols for set ordering, as follows: %
\begin{align}
  A \subset X & \defiif a \in A \Rightarrow a \in X \\
  X \supset A & \defiif A \subset X.
\end{align}
Note that there is no specific symbol for a strict version: If needed, we will explicitely state that $A \neq X$. 
...
a subset $B$ of $X$ is a basis of $X$ \iif the \textit{finite sum} %
\begin{align}
  \bigl\{(z_u)_{u\in B}: z_u \in \C, \singleton{u: z_u \neq 0} \text{ is finite} \bigr\} & \to X \\\nonumber
                                                                                   (z_u) & \mapsto \sum_{z_u \neq 0 } z_u u
\end{align}
bijectively maps all \textit{almost null} $(z_u)$ \textit{onto} $X$. %
%
...
\end{align}
To show the equivalence, we pick a positive $r$ then set $r_i = r/\sqrt{n}$, so that %
\begin{align}
\prod_{i=1}^{n} D_{r_i} \subset B_r. 
\end{align}
%
Conversely, let us choose $r = \min\{r_1, \dots, r_n\}$. Hence %
\begin{align}
