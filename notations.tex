...
%
\begin{align}
	\int_X f \phi \diff{\beta} = \Lambda \phi.
\end{align}
%
...
\end{align}
%
Thus, we define the following spaces %
%
\begin{align}
	L^1 \Def \Biggl\{f: \int_X \magnitude{f} \diff{\beta} < \infty\Biggr\} 
  \subset \Biggl\{f: \forall K \subset X \text{ compact}: \int_{K} \magnitude{f} \diff{\beta} < \infty \Biggr\} 
  \Def L^1_{text{loc}} %
\end{align}
%
...
energy quantum. Thus, the particle contribution is encoded as the Heaviside step function $H(t) = \Iverson{t \geq 0}$, %
