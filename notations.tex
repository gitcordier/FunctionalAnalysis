...
%
Given $A$ and $B$, $A\cup B$ is the union of $A$ and $B$. More generally, for any collection $\Gamma$, the union $U$ of %
$\Gamma$ is expressed as follows:
%
\begin{equation}
U \Def \pmb{\bigcup} \; \Gamma \Def \bigcup_{S \in \Gamma} S.
\end{equation}
%
Similarly, $\cap$ denotes intersection in the same manner. %
%
\subsection{Special mappings}
The identity $I$ (or $\mathrm{id}$) is the mapping $\set{(x,x)}{x\in X}$. Similarly, the projection %
...
$\C^n$ has the standard basis $1_{\singleton{1}},\dots, 1_{\singleton{n}}: \{1, \dots, n\} \to \{0, 1\}$, so that $z_k$ is %
...
\begin{equation}
  ...
  + \cdots
  + z_n  \cdot (\,\underbrace{0, \dots, 1}_{1_{\singleton{n}}}\,) \in \C^n.
\end{equation}
%
A common notation is $e_k = 1_{\singleton{k}}$. Moreover, $\C^n$ is endowed with the product topology generated by all %
polydiscs %
%
\begin{equation}
  \prod_{k=1}^{n} \set{z_k}{\magnitude*{z_k} < r_k} \qquad \bigl(\min(r_1, \dots, r_n) > 0 \bigr).
\end{equation}
%
In the next subsection, we prove that every $n$-dimensional topological vector space is linearly homeomorphic to $\C^n$ %
equipped with this product topology, or, equivalently, with any norm.
%
\subsubsection{Topology of a finite-dimensional vector space}%
To show this, choose an isomorphism $f: \C^n \to Y$. For instance, set $f(e_k) = u_k$, as in \citeFA{1.20}, when $\{u_k\}$ %
...
%
  $\set{f(U)}{U \text{ open in }\C^n}$ \emph{is the unique topological vector space topology on $Y$}. %
%
Thus, $Y$ is necessarily locally convex and locally bounded , \ie,  normable; see \citeFA{1.39}. Note that the equality %
$\norm*[Y]{y}= \norm*[2]{f^{\minus 1}(y)}$ for all $y\in Y$ defines a norm on $Y$. Additionally, $Y$ is locally compact, as %
the closed unit ball of $\C^n$ is compact. Choose an $n$-dimensional topological vector space $W$, and repeat the same %
reasoning with $g: \C^n \to W$, then $h = g\circ f^{\,\minus 1}$, in place of $f$. Thus, $h: Y \to W$ is a homeomorphism and %
...
\begin{enumerate}
  ...
  \item $W$ and $Y$ are linearly homeomorphic and normable.
\end{enumerate}
\renewcommand{\labelenumi}{(\alph{enumi})} 
%
Moreover, the norms on $W$ and $Y$ are \emph{equivalent}. That is, for any given norm $\norm*[Y]{\cdot}$ on $Y$ and %
any given norm $\normtriple{W}{\cdot}$ on $W$, there exists a positive constant $C = C_h$ such that %
%
\begin{equation}
  \normtriple{W}{h(y)} \leq C \normtriple{Y}{y} \qquad\bigl(y \in Y \bigr)
\end{equation}
%
because $h$ is continuous. When $h=\mathrm{id}_Y$, this means that all norms on $Y$ are equivalent, in the sense that %
%
\begin{equation}\label{notations:norm-equivalence}
  \normtriple{Y}{y} \leq C \norm*[Y]{y}.
\end{equation}
%
We now introduce the versions of \eqref{notations:norm-equivalence} where $\norm*[Y]{\cdot}$ and $\normtriple{Y}{\cdot}$ %
denote the standard norms $i, j= 1, 2, \infty$; see \citeFA{1.19}.
%
\subsubsection{The standard norms $1$, $2$, and $\infty$}
When $\C^n$ is equipped with $\norm*[i]{\,\cdot\,}$ and $\norm*[j]{\,\cdot\,}$, the smallest possible constants %
$C_{i, j}$ such that %
%
\begin{equation}
	\norm*[j]{z} \leq C_{i, j} \norm*[i]{z} 
\end{equation}
%
are easily derived from the definitions of these norms, except for the case $C_{2, 1} = \sqrt{n}$, which requires the %
...
\begin{table}[h!]\centering
  ...
  \end{tabular}
  \caption{Optimal $C_{i,j}$ for the norms $i, j = 1, 2, \infty$.}
...
\subsection{Radon measures}\label{notations:Radon-measures}
...
\begin{enumerate}\setcounter{enumi}{1}
  ...
  \begin{equation*}\label{notations:definition-of-Radon-measure}
    \magnitude*{\Lambda \phi} \leq M_K\norm*[\infty]{\phi} \qquad \bigl(\phi \in C_c(X): \supp{\phi} \subset K \bigr).
  \end{equation*}
\end{enumerate}
\renewcommand{\labelenumi}{(\alph{enumi})} 
% 
Condition (ii) defines Radon measures in a more general sense; see \cite{AnalyseIII}. Furthermore, $\Lambda$ is bounded on %
$C_c(X)$ with respect to the supremum norm \IFF (ii) is strengthened by %
$\norm*[\infty]{\Lambda}\leq\sup\set{M_K}{K\text{ compact}} < \infty$. Such uniformly continuous functionals %
...
%
\subsection{Lebesgue integration}\label{notations:Lebesgue-integration}
Theorem \citebook{2.14}{BigRudin} states that every positive Radon measure $\Lambda: C_c(X)\to \C$ is identified with a %
positive and regular Borel measure $\beta:X \to \R$. By ``regular'', we mean that 
%
\begin{equation}
  \sup\set{\beta(K)}{K \subset E,\, K \text{ compact}} = \beta(E) = \inf\set{\beta(U)}{E \subset U,\, U \text{ open}}
\end{equation}
%
when $E$ is measurable. This justifies the following definition of the Lebesgue integral:
%
\begin{equation}\label{definition-of-sum}
	\int_{X} \phi \diff{\beta} \Def \Lambda \phi \qquad \bigl(\phi \in C_c(X)\bigr),
\end{equation}
%
The standard example is $X=\R$ with $\beta=\beta_\R$ the Lebesgue measure on Borel sets. The regularity property implies that %
$\beta$ has total mass
%
\begin{equation}
	\int_{X} 1 \diff{\beta} \Def \sup \set[\big]{\Lambda \phi}{\phi \in C_c(X), 0 \leq \phi \leq 1} \leq \infty.
\end{equation}
%
When we substitute $f\,\beta$ for $\beta$, with $f$ measurable, \eqref{definition-of-sum} takes the form %
%
\begin{equation}\label{Borel-integral}
	\int_{X} \phi \, f \diff{\beta} = \Lambda \phi.
\end{equation}
%
In particular, if $f$ is positive, then $f\,\beta$ is a regular Borel measure such that
%
\begin{equation}
	\int_{X} f \diff{\beta} = \sup \set{\Lambda \phi}{\phi \in C_c(X), 0 \leq \phi \leq 1}.
\end{equation}
%
For general $f$, let $N = N(\beta)$ be the set $\set{u}{u= 0 \;\,\beta\text{-a.e.}}$. Observe that the density $f$ and the %
measure $\Lambda$ are identified modulo $N$ in \eqref{Borel-integral}. Algebraically speaking: %
%
\begin{equation}
...
\begin{equation}
  ...
  \Def \pmb{\bigcap}\; \set[\Big]{\mathcal{L}^1(K,\beta)}{K \text{ compact.}}.
\end{equation}
%
