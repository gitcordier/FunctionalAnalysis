...
\end{align}
Note that we will not reserve any specific symbol for a strict version: If needed, we will explicitly state that $X \neq Y$. 
%
...
with another norm $\norma{{\text{\fw W}}}{\cdot}$, then retrace the latter steps up to (\ref{norm equivalence 1})) with $h$ instead of $f$.%
Hence %
%
\begin{align}
  \norma{{\text{\fw W}}}{w} & \leq C_h \norm{y} \quad\quad \left((y, w) \in h \right)
\end{align}
%
for some $C_h > 0$. The special case $g=f$ is that %
all norms on $Y$ are equivalent, in the sense that %
%
%
\begin{align}
  \norma{{\text{\fw copy of Y}}}{y} \leq C_I \norm{y}.
\end{align}
%
