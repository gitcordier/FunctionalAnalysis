...
see \cite{AnalyseIII}. Moreover, $\Lambda$ is bounded on $C_c(\R^n)$ with respect to the supremum norm \iif (ii) is %
...
\end{align}
%
Moreover, given $N = \set{f}{f = 0 \text{ almost everywhere with respect to }\beta}$, we see that, %
in \eqref{Borel integral}, density $f$ and measure $\Lambda$ are identified modulo $N$. Algebraically speaking: %
\begin{align}
...
%
\begin{align}
	\mathcal{L}^1 \Def \Biggl\{f: \int_{\R^n}\magnitude{f} \diff{\beta} < \infty\Biggr\} 
  \subset \Biggl\{f: \int_{\R^n} 1_K \magnitude{f} \diff{\beta} < \infty %
  \text{, when $K \subset \R^n$ compact} \Biggr\} 
  \Def \mathcal{L}^1_{\text{loc}}, 
\end{align}
%
where $\mathcal{L}^1$ is equipped with the seminorm 
%
\begin{align}
\magnitude{f}_1 \Def \int_{\R^n} \magnitude{f} \diff{\beta} \quad (f \in \mathcal{L}^1).
\end{align}
%
Notably, $N$ is closed in $\mathcal{L}^1$. This implies that $L^1 = \mathcal{L}^1/N$, equipped with the norm %
%
\begin{align}
	\norma{1}{f + N} \Def \int_{\R^n} \magnitude{f} \diff{\beta} \quad (f \in \mathcal{L}^1)
\end{align}
%
is a Banach space. % 
...
or \textit{Dirac's impulse}. Interpreted as a density, $\delta(t)$ carries unit mass because %
%
\begin{align}
  \int_{\minus \infty}^\infty \delta(t) \diff{t} = \int_{\minus \infty}^\infty \diff{H} = H(\infty) - H(\minus \infty) = 1.
\end{align}
%
This expresses that the collision releases the energy quantum. We now apply this heuristic to the carrier signal $\phi$: %
%
\begin{align}
