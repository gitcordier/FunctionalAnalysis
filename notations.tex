...
logical equivalence $\iff$. All definitions are understood to be \IFF clauses, which is consistent with the fact that every %
definition expresses an equivalence.
%
\subsection{The assignment operator}%
Given $\varit{a}$ and $\varit{b}$, $\triangleq$ is a specific instance of $=$. We say that $a\triangleq b$ \IFF $a$ and $b$ %
are \emph{assumed} to be equal. Typically, $a\triangleq b$ means that $a$ is assigned the pre-existing value $b$ %
(some authors write $a:=b$) but this is not a strict requirement. 
%
\subsection{Iverson brackets}\label{Iverson-notation}
Given a boolean expression $\phi$, $\Iverson{\phi}$ encodes the truth value of $\phi$ as a \texttt{bit}:
%
\begin{align} \nonumber
...
\end{align}
is a bijection of all \emph{finitely supported} families $(z_u)$ to $X$. %
%
...
%
Thus, $Y$ is necessarily locally convex and locally bounded \ie normable; see \citeFA{1.39}. Note that %
$\norm[Y]{y}= \norm[2]{f^{\minus 1}(y)}$ for all $y\in Y$ defines a norm. Additionally, $Y$ is locally compact, as the closed %
unit ball of $\C^n$ is compact. Choose an $n$-dimensional topological vector space $W$, and repeat the same reasoning, first %
with $g: \C^n \to W$, next with $h = g\circ f^{\,\minus 1}$, playing the role of $f$. Thus, $h: Y \to W$ is a homeomorphism %
and $W$ is normable as well. The following assertions are then equivalent in the finite-dimensional context: %
%
...
functionals are then uniformly continuous Radon measures. According to \citebook{6.19}{BigRudin}, each of them can be %
...
\end{equation}
%
A central question in calculus is whether the modulus integral $\magnitude{f}_1$, namely 
%
\begin{equation}
...
\begin{equation}
  ...
  \Def \intersectionOfCollection{\set[\Big]{\mathcal{L}^1(K,\beta)}{K \subset X \text{ is compact.}}}
\end{equation}
%
...
defines a norm (the triangle inequality is lost). In contrast, the counterpart %
$L^\infty = \set{f+N}{\norm[\infty]{f+N} < \infty}$ is equipped with $\norm[\infty]{\cdot}$, where the \emph{quotient-norm}
%
\begin{equation}
  \norm[\infty]{f+N} \Def \inf\set[\Big]{M}{M > 0, \magnitude{f} < M \;\,\beta \text{-a.e.}}
\end{equation}
%
