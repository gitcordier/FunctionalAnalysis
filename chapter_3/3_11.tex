...
\newline\newline\noindent
This is actually a consequence of the lemma below,  %
...
%
is $\weakstar$continuous, see \citeFA{3.14}. %
Therefore, %
%
  $P_x= \set[\Big]{\Lambda \in X^\ast}{\magnitude*{\Lambda x} \leq 1}$ %
%
is $\weakstar$ closed: %
...
%
  $X^\ast$ (see \citeFA{3.11}). %
%
Put %
%
  \begin{align}
    W_{F, x} \Def \bigcap_{x\in F} \set[\Big]{\Lambda \in X^\ast}{\magnitude*{\Lambda x} < r_x} \qquad\qquad (r_x > 0)
  \end{align}
%
...
(see \citeFA{1.21 (b)} for a proof): %
By the Hahn-Banach theorem \citeFA{3.5}, %
...
%
  \begin{align}
    B_n \Def \set{x\in X}{d(0, x) <  1/n}\qquad\qquad (\counting{n}) 
  \end{align}
%
...
%
  ...
  \end{align}
%
for some $(n, m) \in \singleton{1, 2, 3, \dots}^2$, see \citeFA{1.18}. %
%
Hence, $X^\ast$ is the countable union of all % 
%
  \begin{align}\label{3.11. Countable union.}
    m\cdot K_n \qquad\qquad (\counting{m,n}), 
  \end{align}
%
...
%
  ...
  \end{align}
%
Since the multiplication by $m$ is a homeomorphism (see \citeFA{1.7}), %
