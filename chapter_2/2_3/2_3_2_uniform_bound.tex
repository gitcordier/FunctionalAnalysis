...
%
  \begin{enumerate}
    ...
    %
    \item{
      $Lambda_n\phi \tendsto{n}{\infty} \Lambda \phi$ for all $\phi$, 
        \ie 
...
%
\begin{align}
  \magnitude{\Lambda \phi} < 1/2 & \quad \text{if }\phi\in V_i, \\
  \magnitude{\Lambda \phi - \lambda_n \phi}   < 1/2 & \quad \text{if } \phi\in V_j.
\end{align}
%
Choose $p\geq i, j $ so that $V_p \subset V_i \cap V_j$: %
The latter inequalities imply that %
%
\begin{align}
  \magnitude{\Lambda \phi} & \leq \magnitude{\Lambda\phi - \Lambda_n \phi} + \magnitude{\Lambda \phi} < 1 \quad\text{if } \phi\in V_p. %
 \end{align}
%
Note that, at arbitrary $\epsilon > 0$, the mapping $\phi_\epsilon = 2^{\minus p}\phi / (\norma{p}{\phi} + \epsilon)$ is a member of $V_p$. %
It is now clear that each below statement is true and implies the following one %
%
\begin{align}
  \magnitude{\Lambda_n \phi_\epsilon} & < 1 \\
  \magnitude{\Lambda_n \phi} & < 2^p \left(\norma{p}{\phi} + \epsilon\right)\\
  \magnitude{\Lambda_n \phi} & \leq 2^p \norma{p}{\phi} \\
  \magnitude{\Lambda_n \phi} & \leq 2^p \left(\norma{\infty}{\phi} + \norma{\infty}{D \phi} + \cdots + \norma{\infty}{D^p \phi}\right).
\end{align}
%
Finally, we apply \eqref{2.3.mean-value-inequality.} with $\phi, D\phi, \dots, D^p \phi$ to obtain %
%
\begin{align}\label{2.3. Bound with p and M (theoritical).}
  \magnitude{\Lambda_n \phi} \leq 2^p (p+1) \norma{\infty}{D^p \phi}.
\end{align}
%
The first part is so proved, with some $p$ and some $ M = M(p)$. %
\newline\newline\noindent
%
% END