...
%
\begin{equation}
  \magnitude{\int_{\minus 1}^1 f_n (t)\phi (t) \diff{t}}\leq M \norm[\infty]{D^p\phi} \nonumber
\end{equation}
%
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
Equipped with the supremum norm, $\Continuous_K=C(\R) \cap \set{\phi}{\supp\phi\subset K}$ realizes the Banach space $C_c(K)$ %
in $C(\R)$. Each density $f_n$ is then identified with the Radon measure %
%
\begin{align}
...
space $\Continuous_K^*$ is a Banach space as well, by \citeFA{4.1}. Here, we consider only pointwise convergence, which is %
weaker than norm convergence. We prove that the assumed pointwise convergence %
%
\begin{equation}\label{2.3-assumed-pointwise-convergence}
  \Lambda_n \phi \tendsto{n}{\infty} \Lambda\phi \quad (\phi \in \D_K)
\end{equation}
%
...
\begin{align}
  ...
  \big\lvert\hspace{0.33 em}\Lambda_n \phi \phantom{,}\big\rvert &\leq M \norm[\infty]{\phi'} \label{2.3-norm-p-bound1},\\
...
%
\begin{equation}
  f_n(t) \Def n^3t \Iverson{\minus 1/n \leq t \leq 1/n}.
\end{equation}
%
...
$n \to \infty$. We see that $\norm{\Lambda_n} \leq \norm{\Lambda_n^+} + \norm{\Lambda_n^\minus}$. However, the collection of %
...
\end{equation}
%
We refer to \citeFA{6.19} for a more general scope. At this step, it is already clear that there is no vague convergence for %
the current sequence $\{\Lambda_n\}$: this would, by Theorem \citeFA{2.6}, imply that $\sup_n{\norm{\Lambda_n}}\ < \infty$, %
which would contradict \eqref{2.3-norm-of-lambda-n}. However, we investigate further to establish weaker convergence and %
boundedness in $\tau_K$. From now on, we bound $\phi$ to $\D_K$: The mean value theorem implies that 
%
\begin{equation}
...
\begin{align}
  ...
  & = \phi'(t_n) 
    - \phi'(0)
  - \frac{n^3}{2} \left(\int_{\minus 1/n}^{1/n} t^2 \left((\phi'(t) - \phi'(0))\right)  \diff{t}\right).
\end{align}
%
...
\begin{equation}\label{2.3-convergence-to-dirac}
  \Lambda_n\phi \tendsto{n}{\infty} \minus \frac{2}{3}\delta' \phi \quad (\phi\in \D_K), 
\end{equation}
%
...
\begin{equation}\label{2.3-upper-bound-assumption}
  \magnitude{\Lambda_n \phi} \leq M \norm[\infty]{\phi} \quad (\phi \in \D_K, \, \counting{n}).
\end{equation}
%
Next, we choose %
...
\end{equation}
%
where $\tilde{\phi}_n$ is the function $\phi$ from Lemma [\ref{lemma-derivative-not-bounded-by-magnitude}], with %
$\tau= 1/n = 1/ \omega = 1/A$. So, $\norm[\infty]{\phi_n} < 2$. Nevertheless, for all $n \geq 4$, $\Lambda_n \phi_n$ is now %
%
\begin{align}
  \Lambda_n \phi_n & = \frac{n}{2} \bigl(\,\underbrace{\phi_n(1/n)}_{1} 
  - \underbrace{\phi_n(\minus 1 /n)}_{\minus 1}\,\bigr) 
  - \underbrace{\frac{n^3}{2} \int_{\minus 1/n}^{1/n} t^2 \phi_n'(t) \diff{t}}_{\frac{1}{3}n} = \frac{2}{3}n .
\end{align}
%
Hence %
% 
...
\end{enumerate}
%
We now present a second example, the \emph{derivative}; see \citeFA{6.1}.
%
\begin{align}
  \Lambda_n': \D_K &\to \C\\
...
\end{align}
%
Indeed, we show that $2$ is the smallest suitable $p$. First, we assume, to reach a contradiction, that
%
\begin{equation}
  \magnitude{\Lambda_n \phi'} \leq M \norm[\infty]{\phi'} \quad (\phi \in \D_K, \, \counting{n}).
\end{equation}
