...
\end{equation}
%
We show that the linear form $\Lambda$ is continuous the topology of $\D_K$. We also construct a particular sequence %
$\{\Lambda_n\}$ whose pointwise limit $\Lambda$ is not norm-bounded. By contraposition, Theorem \citeFA{2.8} implies that %
pointwise convergence on $\D_K$ does not extend to $\Continuous_K$. This conclusion also derives from the following bounds:
%
\begin{align}
...
\begin{description}
  ...
  \item[Case of uniform bound $\magnitude*{\Lambda_n(\phi)} \leq M \norm*{D^2\phi}$:]{%
    Where the smallest possible $p$ to hold is $2$}.
\end{description}
%
...
\end{enumerate}
%
We are in the conditions of the Banach-Steinhaus theorem, which supports the following existence proof.
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
\end{equation}
%
This yields $\magnitude*{\Lambda_n(\phi)}<  p\cdot \bigl\{\norm*[p]{\phi} + \epsilon \bigr\}$, %
...
$\closedInterval*{\minus 1}{1}$, and satisfies $0\leq \psi \leq 1$ on $\R$. $\psi$ can be obtained from the general ù
...
\end{equation}
%
A first point is that there is no vague convergence for this $\{\Lambda_n\}$: this would, by Theorem \citeFA{2.6}, imply that %
$\sup_n{\norm*{\Lambda_n}}\ < \infty$, which would contradict \eqref{2.3:norm-of-lambda-n}. However, we investigate further %
to establish weaker convergence and boundedness in $\tau_K$. The mean value theorem implies that 
%
\begin{equation}
...
\end{align}
%
Note that when $\phi' = 1$ in a neighborhood of $0$, \eg,  for $\phi(t)= t \psi(t)$, the latter equality reduces to %
%
\begin{equation}
...
%
\begin{equation}
  \magnitude*{\Lambda_n(\phi)} \leq \magnitude*{\phi'(t_n)} + \frac{1}{3} \max_{\closedInterval*{\minus 1}{1}}{\magnitude*{\phi'}}.
\end{equation}
%
A simpler bound is
%
...
\end{equation}
%
The behavior $\frac{2}{3}n \to\infty$ brings the desired contradiction. Combining \eqref{2.3:convergence-to-dirac} with %
...
%
\begin{equation}
  \Lambda(\psi \phi_\omega) = \frac{2}{3} \omega \tendsto{\omega}{\infty} \infty 
\end{equation}
%
contrasts with $\norm*[\infty]{\psi \phi_\omega}=1$. Thus, we have exhibited a sequence of Radon measures $\mu_n$ that 
%
\begin{enumerate} 
  \item{
...
\end{enumerate}
%
We now propose a second example, with $2$ as minimal $p$.
%
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Fifth part: Second Counterexample
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\paragraph{Case of uniform bound $\magnitude*{\Lambda_n(\phi)} \leq M \norm*{D^2\phi}$.} We define the \emph{derivative}
%
\begin{align}
  \Lambda_n': \D_K &\to \C\\
...
[1.17]. Therefore, $\Lambda_n'$ is continuous. Now apply \eqref{2.3:convergence-to-dirac} with $\phi'$ and so obtain %
%
\begin{equation}
