...
%
  \begin{equation}\label{2.06. Inner product.}
    \hat{f}(n) 
      = 
    (f,e_n) 
      =
  ...
  \end{equation}
%
We express $\Lambda_n(f)$ as follows: 
\begin{equation}\label{2.06. Rewriting of Lambda_n.}
  \Lambda_n (f) = (f, e_{\minus n}) + \cdots + (f, e_n).
\end{equation}
Moreover, a well-known (and easy to prove) result is % 
%
  \begin{equation}\label{2.06. Orthonormality.}
...
So,  %
%
  \begin{equation}\label{2.06. Norm of Lambda_n.}
    \norm*{\Lambda_n}^2
...
Now suppose, to reach a contradiction, that %
%
  \begin{equation}
...
%
  \end{equation}
%
This establishes that $B$ is actually of the first category; %
and so is its subset %
%
  L=$\set{f\in {L}^2(T)}{\lim_{n\longrightarrow\infty}\Lambda_n f \text{ exists}}$. %
%.
It remains to prove that $L$ is nevertheless dense in $L^2(T)$. %
...
Thus, 
%
  \begin{equation}\label{2.06. Basic inclusions.}
