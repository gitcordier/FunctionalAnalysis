We will actually prove more by showing that $\mathcal{D}(\Omega)$ is separable for each nonempty open subset $\Omega\,$ of $\R^n\,$. 
\begin{proof}
The following is split in three parts. The first one is about the above requested result: %
That was our first part. We now go further by proving the separability of $\mathcal{D}(\Omega)\,$. To do so, we keep $(\alpha,\, j\,)$ in $\N^n\times \N\,$. Remark that $S$ encloses $\text{supp}(D^{\, \alpha}f\,) $: according to the first part, there exists a sequence $\{ P_{\alpha,\, j}:\, j\in \N \}  \subset\R [X_1,\dotsc ,\, X_n]$ such that 
\begin{align}\label{6_1_4}
\|  D^{\, \alpha} f-  \psi P_{\alpha,\, j \,} \|_\infty \underset{j\infty}{\longrightarrow} 0 \qquad . 
\end{align}
Now let $m$ range over $\{1,\, 2,\,3,\, \dots\}$ and set $W_{m,\, j\,}$ in $\mathcal{D}(\Omega)$ as follows
\begin{align}\label{6_1_5a}
D^{\,\minus \alpha} \phi \in \mathcal{D}(\Omega):\, D^{\,\alpha} D^{\,\minus \alpha} \phi= \phi\qquad .
\end{align}
...
\|D^{(m,\dotsc,\, m)} (f   -W_{m,\, j})   \|_\infty < 1/m  \qquad (j\geq k(\!m) )\qquad  .
\end{align}
 Assume without loss of generality that $S$ has diameter $1$so that (\ref{6_1_6}) yields
\begin{align}\label{6_1_7}
\|D^{\, \lambda } (f   -W_{m,\, k(\!m)}  )  \|_\infty < 1/m \qquad ( \lvert \lambda \rvert \leq m )\qquad ,
%\max \{  \lvert D^{\, \lambda}(f- W_{m,\, k(\!m) })(x\,)\rvert :\, x\in S,\, \lvert \lambda\rvert \leq m\,\} < 1/m \qquad 
\end{align}
by the mean value theorem. In other words $(\text{remark that }\text{supp} (f   -W_{m,\, k(\!m)}) \subset S\,)$,  
\begin{align}\label{6_1_8}
f- W_{m,\, k(\!m)}  \in U_m \Def \{ \phi \in \mathcal{D}_S :\, \| \phi \|_m< 1/m \} \supseteq  U_{m+1}\supseteq  \dotsb \qquad( m=1,\, 2,\, 3,\, \dots) \qquad  .
\end{align}
Pick $W$ in $\beta$ (see (b) of [6.3]): $W\cap\mathcal{D}_S$ contains a neighborhood of $0$. Hence $W$ contains some $U_{m\,}$, for $m$ sufficiently large. Thus 
\begin{align}\label{6_1_9}
W_{m,\, k(\!m) } \underset{m\infty} {\longrightarrow} f  \qquad  (\text{in } \mathcal{D}(\Omega))\qquad .
\end{align}
We have so established that the $W_{m,\, k(\!m)}$'s family is dense in $\mathcal{D}(\Omega)$. We now aim to disclose a  countable set $\tilde{W}\,$ that has the same property.\\
\\
Choose $\delta $ in $\R_+$ and fetch any $W_{m,\, k(\!m)}$. Let $X$ be $(X_1,\dotsc,\, X_n)$ and express $P_{(m,\dotsc,\, m\,),\, k(\!m)}$ as 
\begin{align}\label{6_1_10}
P(X\,)=\sum_{\lvert \gamma \rvert \leq d} p_\gamma \cdot X^{\,\gamma} \qquad  .
\end{align}
...
\lvert P(x\,)-Q(x\,) \,\rvert \leq   \sum_{\lvert \gamma \rvert \leq d} \lvert p_\gamma-q_\gamma\rvert\, \lvert x\,\rvert^{\lvert \gamma\rvert} \leq \delta \sum_{l  \leq d} \binom{l+n-1}{n-1}\, \| \, x\, \|^{l}_\infty \qquad (x\in \R^n)\qquad .
\end{align}
Since $S$ is bounded, we so obtain
\begin{align}\label{6_1_12}
 \|\psi (P- Q) \|_\infty  \in O(\delta) \qquad .
\end{align}
...
\begin{align}\label{6_1_13}
 \| D^{\,\lambda} (W_{m,\, k(\!m)} - \tilde{W}_m ) \|_\infty  \in O(\delta)\qquad (\lvert \lambda \rvert\leq m) \qquad   . 
\end{align}
