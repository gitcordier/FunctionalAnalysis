...
\begin{enumerate}
  \item Prove that $\phi$ is a homeomorphism of $(X,\, \tau)$ onto a dense subspace of $(X^{\,\ast\ast},\, \sigma)$.
...
\end{align}
%
is a homeomorphism (with respect to $\tau$ and $\sigma$). We first consider
\begin{align}
  V \Def & \set{
...
\end{align}
and remark that the so defined $V\,$'s (respectively $U\,$'s) shape a local subbase $\mathcal{S}_\sigma$ (respectively $\mathcal{S}_\tau$) of $\sigma$ (respectively $\tau$). We now observe that 
\begin{align}
U=\phi^{\minus 1} \left(V \cap \phi(X\,) \right) = \phi^{\minus 1} (V)  \cap X \qquad ( V\in \mathcal{S}_\sigma\,,\,\,U\in \mathcal{S}_\tau) \qquad ,
\end{align}
since $\phi^{\minus 1}$ is one-to-one. This remains true whether we enrich each subbase $\mathcal{S}$ with all finite intersections of its own elements, for the same reason. It then follows from the very definition of a local base of a weak / weak$^\ast$-topology that $\phi^{\minus 1}$ and its inverse $\phi$ are continuous.\\
\\
The second part of (a) is a special case of [3.5] and is so proved. First, it is evident that 
\begin{align}
\overline{\phi(X\,)}_\sigma\subset X^{\,\ast\ast}\qquad .
\end{align}
and we now assume- to reach a contradiction- that $(X^{\,\ast\ast},\, \sigma)$ contains a point $z^{\,\ast\ast}$ outside the $\sigma$-closure of $\phi(X\,)$. By [3.5], there exists $y^{\,\ast}$ in $X^{\,\ast}$ such that 
\begin{align}
\label{4_1_6} \langle \phi x,\, \psi y^{\,\ast} \rangle=&\langle  y^{\,\ast} ,\,\phi x \rangle= \langle x,\, y^{\,\ast}\rangle= 0\qquad (x\in X\,)\qquad ; \\
...
\begin{align}
\phi(B\,)\subset  B^{\,\ast\ast}\qquad ,
\end{align}
since $\phi$ is isometric. Hence
\begin{align}
\overline{\phi(B\,)}_\sigma\subset \overline{ (B^{\,\ast\ast})}_\sigma=  B^{\,\ast\ast}\qquad .
\end{align}
...
\begin{align}
 \label{4_1_10} \lvert \psi y^{\,\ast}  \rvert \leq &1 \qquad  \text{on}\,\,\overline{\phi(B\,)}_\sigma \qquad  ; \\
   \label{4_1_11}  \langle z^{\,\ast\ast},\, \psi y^{\,\ast} \rangle > &1\qquad .
\end{align}
It follows from (\ref{4_1_10}) that 
\begin{align}
\lvert \, \psi y^{\,\ast}\,   \rvert \leq 1 \text{  on }\phi(B\,) \text{, ie, } \lvert\, y^{\,\ast}  \, \rvert \leq 1  \text{  on } B\qquad .
\end{align}
We have so proved that 
\begin{align}
y^{\,\ast} \in B^\ast \qquad .
\end{align}
Since $ z^{\,\ast\ast}$ lies in $B^{\,\ast\ast}$, it is now clear that 
\begin{align}
\lvert \langle z^{\,\ast\ast},\,  \psi y^{\,\ast} \rangle\rvert \leq 1\qquad; 
\end{align}
what it contradicts (\ref{4_1_11}), and thus proves (b). We now prove (c).\\
\\
%: c
...
\begin{align}
\phi(B\,)= \overline{\phi(B\,)}_\sigma\overset{(b)}{=} B^{\,\ast\ast}\qquad .
\end{align}
...
\begin{quotation}
\noindent  \emph{Given a reflexive Banach space $Z$, the weak$^{\,\ast}$-topology of $Z^{\,\ast}$ is its weak one.}
\end{quotation}
